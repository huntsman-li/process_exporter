...
func GetConfig(content string) (*Config, error) {
	...
	for i, procname := range procnames {
		mn, names, err := getMatchNamer(procname)
		if err != nil {
...
func getMatchNamer(yamlmn interface{}) (common.MatchNamer, []string, error) {
	nm, ok := yamlmn.(map[interface{}]interface{})
	if !ok {
		return nil, nil, fmt.Errorf("not a map")
	}
	...
	for k, v := range nm {
		...
		if key == "name" {
			value, ok := v.(string)
			if !ok {
				return nil, nil, fmt.Errorf("non-string value %v for key %q", v, key)
			}
			nametmpl = value
		} else {
			vals, ok := v.([]interface{})
			if !ok {
				return nil, nil, fmt.Errorf("non-string array value %v for key %q", v, key)
			}
			var strs []string
			for i, si := range vals {
				s, ok := si.(string)
				if !ok {
					return nil, nil, fmt.Errorf("non-string value %v in list[%d] for key %q", v, i, key)
				}
				strs = append(strs, s)
			}
			smap[key] = strs
	...
	if cmdline, ok := smap["cmdline"]; ok {
		...
		for _, c := range cmdline {
			...
			if err != nil {
				return nil, nil, fmt.Errorf("bad cmdline regex %q: %v", c, err)
			}
			rs = append(rs, r)
		}
	...
	if len(matchers) == 0 {
		return nil, nil, fmt.Errorf("no matchers provided")
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("bad name template %q: %v", nametmpl, err)
	}

	return &matchNamer{matchers, templateNamer{tmpl}}, names, nil
}
